...
abstract class BasesfSimpleCMSSlot extends BaseObject implements Persistent
{
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 7; // 7 = sfSimpleCMSSlotPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = sfSimpleCMSSlotQuery::create()
                ->filterByPrimaryKey($this->getPrimaryKey());
            $ret = $this->preDelete($con);
            // archivable behavior
            if ($ret) {
                if ($this->archiveOnDelete) {
                    // do nothing yet. The object will be archived later when calling sfSimpleCMSSlotQuery::delete().
                } else {
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($isInsert) {
                ...
                if (!$this->isColumnModified(sfSimpleCMSSlotPeer::UPDATED_AT)) {
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(sfSimpleCMSSlotPeer::PAGE_ID)) {
            $modifiedColumns[':p' . $index++]  = '`page_id`';
        }
        if ($this->isColumnModified(sfSimpleCMSSlotPeer::CULTURE)) {
            $modifiedColumns[':p' . $index++]  = '`culture`';
        }
        if ($this->isColumnModified(sfSimpleCMSSlotPeer::NAME)) {
            $modifiedColumns[':p' . $index++]  = '`name`';
        }
        if ($this->isColumnModified(sfSimpleCMSSlotPeer::TYPE)) {
            $modifiedColumns[':p' . $index++]  = '`type`';
        }
        if ($this->isColumnModified(sfSimpleCMSSlotPeer::VALUE)) {
        ...
        $sql = sprintf(
            'INSERT INTO `sf_simple_cms_slot` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['sfSimpleCMSSlot'][serialize($this->getPrimaryKey())])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['sfSimpleCMSSlot'][serialize($this->getPrimaryKey())] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->asfSimpleCMSPage) {
                $result['sfSimpleCMSPage'] = $this->asfSimpleCMSPage->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
     ...
     * @throws PropelException
     */
    public function getsfSimpleCMSPage(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->asfSimpleCMSPage === null && ($this->page_id !== null) && $doQuery) {
            $this->asfSimpleCMSPage = sfSimpleCMSPageQuery::create()->findPk($this->page_id, $con);
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->asfSimpleCMSPage instanceof Persistent) {
              $this->asfSimpleCMSPage->clearAllReferences($deep);
            }
    ...
    {
        return (string) $this->exportTo(sfSimpleCMSSlotPeer::DEFAULT_STRING_FORMAT);
    }
     ...
     * @param PropelPDO $con Optional connection object
     *
     * @return     SfSimpleCmsSlotArchive An archive object, or null if the current object was never archived
     */
    public function getArchive(PropelPDO $con = null)
    {
        ...
        $archive = SfSimpleCmsSlotArchiveQuery::create()
            ->filterByPrimaryKey($this->getPrimaryKey())
            ->findOne($con);
     ...
     *
     * @return     SfSimpleCmsSlotArchive The archive object based on this object
     */
    public function archive(PropelPDO $con = null)
    {
        ...
        if (!$archive = $this->getArchive($con)) {
            $archive = new SfSimpleCmsSlotArchive();
     ...
     *
     * @param      SfSimpleCmsSlotArchive $archive An archived object based on the same class
      *
     * @return     sfSimpleCMSSlot The current object (for fluent API support)
     */
    public function populateFromArchive($archive) {
        $this->setPageId($archive->getPageId());
        $this->setCulture($archive->getCulture());
        $this->setName($archive->getName());
        $this->setType($archive->getType());
        $this->setValue($archive->getValue());
    ...
    public function __call($name, $params)
    {
        ...
        // symfony_behaviors behavior
        if ($callable = sfMixer::getCallable('BasesfSimpleCMSSlot:' . $name))
        {
