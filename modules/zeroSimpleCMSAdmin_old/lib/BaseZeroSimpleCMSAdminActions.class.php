...
class BaseZeroSimpleCMSAdminActions2 extends autozeroSimpleCMSAdminActions
{
	...
	public function executeTogglePublish()
	{
		...
		$this->redirect('zeroSimpleCMSAdmin/list?page='.$this->getRequestParameter('page', 1));
	}

	public function executeToggleSecure()
	{
		$page = sfSimpleCMSPagePeer::retrieveByPk($this->getRequestParameter('id'));
		$this->forward404Unless($page);
		...
		$page->setIsSecure(!$page->getIsSecure());
		$page->save();
	...
	{
		$this->forward('zeroSimpleCMSAdmin', 'list');
	}
