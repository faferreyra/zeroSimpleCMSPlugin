<?php use_helper('jQuery', 'I18N', 'Validation', 'sfSimpleCMS', 'sfMediaLibrary') ?>

<div id="editor_box" style="display: none;">
  <h1>
    <?php echo __('Edition Tools') ?>
    <?php echo link_to_function('&nbsp;&nbsp;&nbsp;&nbsp;', "toggle_pane('tools');jQuery('#toolbar_minifier').toggleClass('minimized')", 'id=toolbar_minifier title='.__('Toggle toolbar')) ?>
  </h1>
  ...
  <div id="cms_toolbox_mode">
    ...
    <div>
      ...
      <?php if ($page->getIsPublished()): ?>
        ...
        <span class="unpublished"><?php echo link_to_with_path(__('unpublish'), 'zeroSimpleCMS/togglePublish?slug='.$slug, array('query_string' => $query_string.'&sf_culture='.$culture)) ?></span>
      <?php else: ?>
        <span class="published"><?php echo link_to_with_path( __('publish'), 'zeroSimpleCMS/togglePublish?slug='.$slug, array('query_string' => $query_string.'&sf_culture='.$culture)) ?></span>
  ...
  <div id="tools" class="open">
    ...
    <?php echo form_tag('zeroSimpleCMS/update', 'name=cms_tools_update_page id=cms_tools_update_page class=open') ?>
      ...
      <div class="form_control">
        ...
        <?php echo javascript_tag('
        	var	$current_page_option = jQuery(\'#create_position option[selected=selected]\');
        	$current_page_option.val(\'\');
        $current_page_option.addClass(\'current\');
        ') ?>
      </div>
  
      <div class="form_submit">
        <?php echo button_to_with_path(__('delete'), 'zeroSimpleCMS/delete?sf_culture='.$culture.'&slug='.$slug, array(
    ...
    <?php echo form_tag('zeroSimpleCMS/create', 'name=cms_tools_create_page id=cms_tools_create_page style=display:none') ?>
